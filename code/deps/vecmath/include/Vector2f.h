...
public:
    ...
    float abs() const;		// 返回绝对值(长度)
    float absSquared() const;
    void normalize();		// 化为单位向量
    Vector2f normalized() const;	// 返回单位向量
	...
	// returns v0 * ( 1 - alpha ) * v1 * alpha 	// 线性差值
...
Vector2f operator - ( const Vector2f& v0, const Vector2f& v1 );
Vector2f operator * ( const Vector2f& v0, const Vector2f& v1 );
Vector2f operator / ( const Vector2f& v0, const Vector2f& v1 );	// 这很奇怪
