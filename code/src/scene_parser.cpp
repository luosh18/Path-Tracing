...
void SceneParser::parsePerspectiveCamera() {
    ...
    Vector3f center = Vector3f::ZERO, direction = Vector3f::FORWARD, up = Vector3f::UP;
    float width = 1024, height = 768, angle_degrees = 30, aperture = 0, focus = 100;
    getToken(token);
    assert (!strcmp(token, "{"));
    while(true) {
        getToken(token);
    ...
    camera = new PerspectiveCamera(center, direction, up, width, height, DegreesToRadians(angle_degrees), aperture, focus);
}
...
Material *SceneParser::parseMaterial() {
    ...
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), refractColor(0, 0, 0), emission(0, 0, 0);
    float diffuse = 1, specular = 0, refractiveIndex = 1, refract = 0;
    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
        getToken(token);
    ...
    auto *answer = new Material(diffuseColor, diffuse, specularColor, specular, refractColor, refractiveIndex, refract, emission);
    return answer;
}
...
BezierCurve *SceneParser::parseBezierCurve() {
    ...
    while (true) {
        getToken(token);
        if (!strcmp(token, "[")) {
            controls.push_back(readVector2f());
            getToken(token);
            assert (!strcmp(token, "]"));
...
RevSurface *SceneParser::parseRevSurface() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    auto *answer = new RevSurface(pos, profile, current_material);
    return answer;
}
